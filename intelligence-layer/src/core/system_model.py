...
def render_for_prompt() -> str:
    """Render system model as production-language Markdown for LLM system prompt."""
    parts: list[str] = ["## DATACENTER SYSTEM MODEL\n"]

    parts.append("### Trust Zones (4)\n")
    ...
    for l in LEAFS.values():
        parts.append(f"- **{l.name}** mgmt {l.mgmt_ip} — manages zones {l.zones}. {l.role}")
